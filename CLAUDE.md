...
beta.rocks is a free, chat-first web app that provides climbing conditions, weather forecasts, and community reports for any crag worldwide. It uses AI (Google Gemini 2.5 Flash via Vercel AI SDK) to provide real-time conditions, friction scores, and route information based on weather data, rock type, and climbing-specific factors. Features include user profiles with multi-device sync, chat history persistence, favorites bookmarking, and community reports with multi-select categories.
