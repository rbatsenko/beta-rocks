...
interface HeaderProps {
  ...
  actions?: ReactNode;
  /** Whether to show the live indicator (for realtime features) */
 ...
 */
export function Header({ actions, showLiveIndicator = false }: HeaderProps) {
  return (
    <header className="border-b bg-background/95 backdrop-blur-sm supports-backdrop-filter:bg-background/60 sticky top-0 z-50">
      <div className="container flex h-16 items-center justify-between px-4">
        <div className="flex items-center gap-3">
          ...
          <LiveIndicator isLive={showLiveIndicator} compact />
        </div>
