-- Update RPC function to return categories array instead of single category
...
RETURNS TABLE (
  ...
  route_id text,
  author_id text,
  categories text[],  -- Changed from category text to categories text[]
  text text,
  rating_dry integer,
  rating_wind integer,
...
BEGIN
  ...
  SELECT
    ...
    r.route_id,
    r.author_id,
    r.categories,  -- Changed from r.category to r.categories
    r.text,
    r.rating_dry,
    r.rating_wind,
    r.rating_crowds,
...
'Fetches reports for a crag with proper date-grouped sorting and multi-select categories. Groups reports by observation date (ignoring time), then sorts by creation timestamp within each day. All ID fields use TEXT type to match schema.';
