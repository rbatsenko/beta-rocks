-- Revert RPC function to return single category instead of categories array
...
RETURNS TABLE (
  ...
  route_id text,
  author_id text,
  category text,  -- Changed from categories text[] back to category text
  text text,
  rating_dry integer,
  rating_wind integer,
...
BEGIN
  ...
  SELECT
    ...
    r.route_id,
    r.author_id,
    r.category,  -- Changed from r.categories back to r.category
    r.text,
    r.rating_dry,
    r.rating_wind,
    r.rating_crowds,
...
'Fetches reports for a crag with proper date-grouped sorting and single category. Groups reports by observation date (ignoring time), then sorts by creation timestamp within each day. All ID fields use TEXT type to match schema.';
