-- Update search_locations_enhanced to return sector slugs instead of null
...
BEGIN
  ...
  SELECT * FROM (
    ...
    -- Search sectors (with parent crag info, now including sector slug)
    SELECT
      s.id,
      s.name,
      ...
      s.description,
      s.source,
      s.slug::TEXT as slug,  -- Return sector's own slug
